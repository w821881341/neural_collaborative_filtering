...
def get_model(train_matrix,num_users, num_items, layers=[20, 10], reg_layers=[0, 0]):
    ...
    cost_layer.add(Lambda(lambda x: K.sum(K.square(x[0] - x[1][:, 0]), 1, keepdims=True), input_shape=(2,num_items,), name='user_reconstruct_cost'))
    cost_layer.build((2,num_items,))
