...
def get_model(train_matrix,num_users, num_items, layers=[20, 10], reg_layers=[0, 0]):
    ...
    user_encoder = Dense(layers[0] , input_dim=num_items, activation='relu')
    user_decoder = Dense(num_items , input_dim=layers[0], activation='relu')
    item_encoder = Dense(layers[0], input_dim=num_users, activation='relu')
    item_decoder = Dense(num_users , input_dim=layers[0], activation='relu')

    user_encoder_MLP = user_encoder(user_data)
    # user_encoder_MLP.build((num_items,))
    user_decoder_MLP = user_decoder(user_encoder_MLP)
    # user_decoder_MLP.build((layers[0],))
    item_encoder_MLP = item_encoder(item_data)
    # item_encoder_MLP.build((layers[0],))
    item_decoder_MLP = item_decoder(user_encoder_MLP)
    # item_decoder_MLP.build((layers[0],))
