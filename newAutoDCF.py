...
def get_model(train_matrix,num_users, num_items, layers=[20, 10], reg_layers=[0, 0]):
    ...
    user_cost = K.sum(merge([K.square(user_data - user_decoder_MLP),user_data],mode='mul'), axis=-1)/K.sum(user_data,axis=-1)
    item_cost = K.sum(merge([K.square(item_data - item_decoder_MLP),item_data],mode='mul'), axis=-1)/K.sum(item_data,axis=-1)
